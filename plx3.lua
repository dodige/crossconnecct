...
function on_end_file()
    ...
    -- Check if the current file is the last file in the playlist
    print(current_pos - 0  == playlist_count - 1 ) 
    if current_pos - 0  == playlist_count - 1 then
        -- End of playlist reached
        msg.info("End of playlist reached.")
--        msg.info(mp.get_property("playlist"))
        ...
        local filename = "/home/pi/medias/playlistupdating.txt"

        -- Check if the file exists in the current directory
        --if lfs.attributes(filename) then
        local f = io.open(filename, "r")
        if f ~= nil then
          -- Print the message skipping
          print("Skipping")
        else
            -- Print the message replacing
            print("Replacing")      
            mp.commandv("playlist-clear")
            mp.commandv("loadlist", "/home/pi/medias/playlist.m3u" , "replace")
...
-- Register the "end-file" event
mp.register_event("end-file", on_end_file)


mp.command("set loop-playlist yes")
